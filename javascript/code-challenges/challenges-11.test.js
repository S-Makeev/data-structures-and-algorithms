...
function transformToLis(obj){
  ...
  return newArr.map(([key, value]) => `<li>${key}: ${value}</li>`);
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((form, array) => {
    return form + array.filter((element) => element === target).length;
  }, 0);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
function totalSum(input) {
  ...
  return input.reduce((total, arr) => {
    return total + arr.reduce((sum, el) => {
      return sum + el;
    }, 0);
  }, 0);
}
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((char) => ['male', 'female'].includes(char.gender))
    .map((char) => char.name)
    .join(' and ');
};
